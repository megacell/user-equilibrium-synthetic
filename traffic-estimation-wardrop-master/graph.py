...
class Graph:
    ...
    def update_linkflows_from_cpp_output(self, cpp_output):
        dict = {}
        for link_output in cpp_output:
            startnode, endnode, flow = link_output[0], link_output[1], link_output[2]
            dict[(startnode, endnode, 1)] = flow
        for id,link in self.links.items():
            flow = dict[id]
            cap = 1/link.delayfunc.slope
            link.flow, link.delay = flow, link.delayfunc.compute_delay(flow) 
            #link.flow, link.delay = flow, link.ffdelay * min((1 + 0.15*np.power(flow/cap,4)),4)
            if link.delay/link.ffdelay > 5: link.delay = link.ffdelay
